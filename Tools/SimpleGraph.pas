...
{ ------------------------------------------------------------------------------ }
{
    2018-09-25 修改PAN模式滚动方法。原版方法是通过分别设置横竖两个滚动条滑块位置、
        在TGraphScrollBar.SetPosition方法中用Owner.ScrollBy(...)实现的。问题是他
        分别设置，导致pan的时候，先横滚一下，再竖滚一下，哆哆嗦嗦的。平滑的方式是
        直接用ScrollBy（dx，dy）。所以本次修改了GraphScrollBar.SetPosition方法和
        SimpleGraph.MouseMove方法，参见这两个方法的注释。
}
{$I DELPHIAREA.INC}
...
type
  ...
  { TGraphScrollBar -- for internal use only }
    ...
    private
        ...
        fDontScroll   : Boolean; // 2018-09-25 标志：别滚动！！
    ...
    published
        ...
        property Increment: TScrollBarInc read fIncrement write fIncrement stored IsIncrementStored
            default 8;
        ...
        property DontScroll : Boolean read fDontScroll write fDontScroll default False;
    end;
  ...
  { TGraphObject }
    ...
    protected
        ...
        procedure UpdateDependencyTo(GraphObject: TGraphObject;
        ...
        procedure DependentChanged(GraphObject: TGraphObject;
            Action: TGraphObjectListAction); virtual;
        procedure LinkInputChanged(GraphObject: TGraphObject;
            Action: TGraphObjectListAction); virtual;
        procedure LinkOutputChanged(GraphObject: TGraphObject;
            Action: TGraphObjectListAction); virtual;
...
{$IFDEF COMPILER5_UP} override; {$ENDIF}
        ...
        procedure DoObjectEndDrag(GraphObject: TGraphObject; HT: DWORD;
        ...
        procedure DoObjectUnhook(GraphObject: TGraphObject; Link: TGraphLink;
            Index: Integer); virtual;
        procedure DoObjectHook(GraphObject: TGraphObject; Link: TGraphLink;
            Index: Integer); virtual;
    ...
    published
        ...
        property Anchors;
        property BiDiMode;
        property ClipboardFormats: TGraphClipboardFormats read fClipboardFormats
            write fClipboardFormats default [cfNative];
        property Color;
        ...
        property Enabled;
        property FixedScrollBars: Boolean read fFixedScrollBars write SetFixedScrollBars
            default False;
        property Font;
        property GraphConstraints: TGraphConstraints read fGraphConstraints
            write SetGraphConstraints;
        ...
        property ShowHiddenObjects: Boolean read fShowHiddenObjects write SetShowHiddenObjects
            default False;
        property ShowHint;
...
{$ENDIF}
        ...
        property OnUnDock;
        property OnObjectInitInstance: TGraphNotifyEvent read fOnObjectInitInstance
            write fOnObjectInitInstance;
        ...
        property OnObjectContextPopup: TGraphContextPopupEvent read fOnObjectContextPopup
            write fOnObjectContextPopup;
        property OnObjectBeforeDraw: TGraphObjectDrawEvent read fOnObjectBeforeDraw
            write fOnObjectBeforeDraw;
        property OnObjectAfterDraw: TGraphObjectDrawEvent read fOnObjectAfterDraw
            write fOnObjectAfterDraw;
        property OnObjectBeginDrag: TGraphBeginDragEvent read fOnObjectBeginDrag
            write fOnObjectBeginDrag;
        property OnObjectEndDrag: TGraphEndDragEvent read fOnObjectEndDrag write fOnObjectEndDrag;
        property OnObjectMouseEnter: TGraphNotifyEvent read fOnObjectMouseEnter
            write fOnObjectMouseEnter;
        property OnObjectMouseLeave: TGraphNotifyEvent read fOnObjectMouseLeave
            write fOnObjectMouseLeave;
        ...
        property OnCanLinkObjects: TGraphCanLinkEvent read fOnCanLinkObjects
            write fOnCanLinkObjects;
        property OnCanMoveResizeNode: TCanMoveResizeNodeEvent read fOnCanMoveResizeNode
            write fOnCanMoveResizeNode;
        property OnCanRemoveObject: TGraphCanRemoveEvent read fOnCanRemoveObject
            write fOnCanRemoveObject;
        property OnNodeMoveResize: TGraphNodeResizeEvent read fOnNodeMoveResize
            write fOnNodeMoveResize;
        property OnGraphChange      : TNotifyEvent read fOnGraphChange write fOnGraphChange;
        property OnCommandModeChange: TNotifyEvent read fOnCommandModeChange
            write fOnCommandModeChange;
...
var
    ...
    begin
        if Control.Visible then
            case Control.Align of
                alLeft, alNone:
                    if (Control.Align = alLeft) or (Control.Anchors * [akLeft, akRight] = [akLeft])
                    then
    ...
    begin
        if Control.Visible then
            case Control.Align of
                alTop, alNone:
                    if (Control.Align = alTop) or (Control.Anchors * [akTop, akBottom] = [akTop])
                    then
...
begin
    ...
    begin
      ...
      // 2018-9-25此处添加是否pan模式判断，若是pan模式，则滚动操作在SimpleGraph.MouseMove方法中已实施了。
      // 若不是pan模式，则属于用户用鼠标或方向键滚动滚动条。
            if (Owner.CommandMode <> cmPan) and (not fDontScroll) then
            begin
                if Kind = sbHorizontal then
...
var
    I: Integer;
begin
    DrawControlPoint(Canvas, fPoints[0],
        not(Owner.LockLinks or (gloFixedStartPoint in LinkOptions)));
    for I := 1 to PointCount - 2 do
        DrawControlPoint(Canvas, fPoints[I],
            not(Owner.LockLinks or (gloFixedBreakPoints in LinkOptions)));
    DrawControlPoint(Canvas, fPoints[PointCount - 1],
        not(Owner.LockLinks or (gloFixedEndPoint in LinkOptions)));
end;
            ...
            try
                ...
                begin
                    ...
                    ModifiedPolyline[PointCount - 1] :=
                        DrawPointStyle(Canvas, fPoints[PointCount - 1],
...
begin
    ...
    begin
        ...
        begin
            ...
            OffsetRect(TextRect, fTextCenter.X - TextRect.Right div 2,
                fTextCenter.Y - TextRect.Bottom);
                    ...
                    begin // Workaround Windows bug!
                        ...
                        try
                            ...
                            Bitmap.Canvas.StretchDraw
                                (Rect(0, 0, Bitmap.Width, Bitmap.Height), Graphic);
...
function TRoundRectangularNode.LinkIntersect(const LinkPt: TPoint; const LinkAngle: Double)
    : TPoints;
var
    S: Integer;
begin
...
begin
    ...
    begin
        ...
        begin
      // 2018-09-25下面添加的这句配合TGraphScrollBar.SetPosition中的修改可以实现平滑PAN体验。
      // 在原版的pan模式的实现，是通过分别设置横、竖两个滚动条的位置实现的，于是窗口内容的滚动
      // 是先横向滚一下，再竖向滚一下，感觉在哆嗦。本次修改在mousemove方法中直接用ScrollBy完成
      // X、Y方向的滚动，再在滚动条的SetPosition方法中判断若时pan模式则只设置滑块的位置，并不
      // 滚动窗口，这样就好了。
...
begin
    ...
    //2018-09-25
    scrollby(horzscrollbar.Position-dx,vertscrollbar.Position-dy);
    ...
    try
        ...
        begin
            ...
            try
                ...
                if GraphConstraints.ConfineOffset(OffsetX, OffsetY,
                    [osLeft, osTop, osRight, osBottom]) then
                begin
                    Inc(SuspendQueryEvents);
                    try
                        for I := 0 to ObjectList.Count - 1 do
...
begin
    if Assigned(Org) then
        SetViewportOrgEx(DC, -(Org^.X + HorzScrollBar.Position),
            -(Org^.Y + VertScrollBar.Position), nil)
    else
...
function TSimpleGraph.AlignSelection(Horz: THAlignOption; Vert: TVAlignOption): Boolean;
    ...
    begin
        ...
        try
            ...
            Space := ObjectList[ObjectList.Count - 1].BoundsRect.Right - ObjectList[0]
                .BoundsRect.Left;
            for I := 0 to ObjectList.Count - 1 do
    ...
    begin
        ...
        try
            ...
            Space := ObjectList[ObjectList.Count - 1].BoundsRect.Bottom - ObjectList[0]
                .BoundsRect.Top;
            for I := 0 to ObjectList.Count - 1 do
                with ObjectList[I].BoundsRect do

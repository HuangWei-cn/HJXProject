...
type
  ...
  /// <remarks>
  /// sptType是LineSeries的Pointer类型（注：不是Style）
  /// </remarks>
  ...
  private
    ...
    // 2019-10-11 -----------
    /// <remarks>
    /// 是否有Pointer，值为None，Auto，Always。当为Auto时，程序根据数据数量决定是否设置LineSeries的
    /// Pointer属性为Visible，若数据数量过多，将隐藏LineSeries的Pointer，改为增加一个PointerSeries
    /// 类型，这个PointerSeries按一定间隔从LineSeries中取数据，以减少Pointer的显示数量。
    /// </remarks>
    FPointerType: TSeriesPointerType;
    /// <remarks>
    /// LineSeries的线型设置，默认为psSolid。但用户可以在模板中指定其他类型。其他类型有psDash,
...
begin
  ...
  // 2019-10-11 检查是否设置了PointerType
  if High(Params) > 2 then
  begin
    if SameText(Params[3], 'NONE') then NewSeries.PointerType := sptNone
    else if SameText(Params[3], 'AUTO') then NewSeries.PointerType := sptAuto
    else if SameText(Params[3], 'ALWAYS') then NewSeries.PointerType := sptAlways
  ...
  begin
    if SameText(Params[4], 'Solid') then NewSeries.LineStyle := slsSolid
    else if SameText(Params[4], 'Dash') then NewSeries.LineStyle := slsDash
    else if SameText(Params[4], 'Dot') then NewSeries.LineStyle := slsDot
    else if SameText(Params[4], 'DashDot') then NewSeries.LineStyle := slsDashDot
    else if SameText(Params[4], 'DashDotDot') then NewSeries.LineStyle := slsDashDotDot

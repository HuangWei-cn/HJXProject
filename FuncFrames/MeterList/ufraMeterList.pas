...
uses
  ...
  Vcl.ComCtrls, {uHJX.Excel.Meters}
  uHJX.Intf.AppServices, uHJX.Classes.Meters, uHJX.Intf.FunctionDispatcher,
...
type
  ...
  TfraMeterList = class(TFrame)
    ...
    N3: TMenuItem;
    actPopTrendLine: TAction;
    N4: TMenuItem;
    piPopTrendLine: TMenuItem;
    actPopDatas: TAction;
    piPopDatas: TMenuItem;
    procedure tvwMetersCreateNodeClass(Sender: TCustomTreeView;
      var NodeClass: TTreeNodeClass);
    procedure edtSearchChange(Sender: TObject);
    procedure tvwMetersCustomDrawItem(Sender: TCustomTreeView; Node: TTreeNode;
      State: TCustomDrawState; var DefaultDraw: Boolean);
    procedure tvwMetersContextPopup(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
    ...
    procedure actPopTrendLineExecute(Sender: TObject);
    procedure actPopDatasExecute(Sender: TObject);
  private
    ...
    FOnPopupTrendLine: TOnMeterOpEvent; // 弹出过程线窗口
    FOnPopupDatas: TOnMeterOpEvent; // 弹出观测数据窗口
  ...
  published
    property OnShowMeterDatas: TOnMeterOpEvent read FOnShowMeterDatas
      write FOnShowMeterDatas;
    property OnShowMeterTrendLine: TOnMeterOpEvent read FOnShowMeterTrendLine
      write FOnShowMeterTrendLine;
    property OnShowMeterVectorGraph: TOnMeterOpEvent
      read FOnShowMeterVectorGraph write FOnShowMeterVectorGraph;
    property OnDblClickMeter: TOnMeterOpEvent read FOnDblClickMeter
      write FOnDblClickMeter;
    property OnPopupTrendLine: TOnMeterOpEvent read FOnPopupTrendLine
      write FOnPopupTrendLine;
    property OnPopupDatas: TOnMeterOpEvent read FOnPopupDatas
      write FOnPopupDatas;
  end;
...
procedure TfraMeterList.actPopDatasExecute(Sender: TObject);
begin
  if Assigned(FOnPopupDatas) and (tvwMeters.Selected <> nil) then
    with tvwMeters.Selected as TmeterNode do
      if Meter <> nil then
        FOnPopupDatas(Meter.DesignName);
end;

procedure TfraMeterList.actPopTrendLineExecute(Sender: TObject);
begin
  if Assigned(FOnPopupTrendLine) and (tvwMeters.Selected <> nil) then
    with tvwMeters.Selected as TmeterNode do
      if Meter <> nil then
        FOnPopupTrendLine(Meter.DesignName);
end;
...
procedure TfraMeterList.actViewDataSheetExecute(Sender: TObject);
var
  IFunc: IFunctionDispatcher;
begin
  if IAppServices = nil then
    Exit;
  if IAppServices.FuncDispatcher = nil then
    Exit;
  if tvwMeters.Selected = nil then
    Exit;
  ...
  with tvwMeters.Selected as TmeterNode do
    ...
    begin
      ...
      if IFunc.HasProc('ShowMeterSheetWithoutExcel') then
        IFunc.CallFunction('ShowMeterSheetWithoutExcel', Meter.DesignName);
    end;
end;
...
procedure TfraMeterList.tvwMetersCustomDrawItem(Sender: TCustomTreeView;

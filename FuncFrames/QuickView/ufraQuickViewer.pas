...
begin
  ...
  begin
    ShowMessage('你得选择某条记录，有工程部位和仪器类型的记录');
    Exit;
  end;
...
{ -----------------------------------------------------------------------------
  Procedure  : ShowQuickView
  Description: 对应着“逐支速览”功能，只用WebGrid进行表示
----------------------------------------------------------------------------- }
...
{ -----------------------------------------------------------------------------
  ...
  物理量。目前本功能只用WebGrid表示，并且显示所有的物理量，行显示，不是EhGrid
  那种用多行显示多物理量的方式。
----------------------------------------------------------------------------- }
